...
# リファレンス
### 通信・データ取得系コマンド 比較表
| コマンド | 主な用途 | デフォルトの出力先 | 強み・設計思想 | セキュリティ検証のタイミング | セキュリティ検証の方式 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **`curl`** | データ転送・API実行 | 標準出力 | 双方向通信、多様なプロトコル、REST API検証 | リモートサーバーとの**通信確立時** | **トランスポート層**: サーバーのSSL/TLS証明書チェーンおよびホスト名の有効性検証。 |
| **`wget`** | ファイルのダウンロード | ファイル保存 | クローラー機能、再開機能、巨大ファイル処理 | リモートサーバーとの**通信確立時** | **トランスポート層**: サーバーのSSL/TLS証明書の有効性検証。 |
| **`apt`** | パッケージ管理 | 内部処理・システム構築 | 依存関係の解決、暗号学的な完全性検証 | メタデータ/パッケージ**取得後**、インストール実行前 | **アプリケーション層**: GPG公開鍵によるReleaseファイルのデジタル署名検証。（※HTTPSリポジトリ指定時は通信確立時のTLS検証も並行して行われる） |
| **`nc`** | ネットワーク検証・通信 | 標準出力 / 任意 | TCP/UDPの直接操作、ポート疎通確認 | **なし** | プレーンテキスト通信。プロトコルの直接操作が目的のため、暗号化や検証機能は持たない。 |
| **`rsync`** | ファイル同期・転送 | ファイル/ディレクトリ | 差分転送アルゴリズムによる高速な同期 | リモートホストとの**接続確立時** | **セッション層**: 外部シェル（標準ではSSH）に依存。SSHによるホスト公開鍵の検証と通信経路の暗号化。 |