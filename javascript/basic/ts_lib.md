...
| Property | 説明 |
| :--- | :--- |
| `required_error` | 値が `undefined`（未送信や空）の場合に出力される必須エラーメッセージ。 |
| `invalid_type_error` | 要求した型と実際の値の型が異なる場合に出力される型エラーメッセージ。 |
| `message` | 制約メソッド（文字数、最大値、形式など）の条件を満たさない場合に出力されるエラーメッセージ。 |
| `description` | スキーマの用途などを記述する補足説明（エラー表示用ではなく、メタデータとして使用）。 |
...
const UserFormSchema = z.object({
  ...
  age: z.coerce.number({
    ...
    invalid_type_error: "年齢には数値を入力してください",
  })
  .gte(18, { message: "18歳未満は登録できません" })
  .optional(), // 任意入力項目にする場合
});

// 実行例
const result = UserFormSchema.safeParse({
  username: "ab",       // エラー: 3文字未満
  email: "invalid-mail",// エラー: email形式ではない
  age: "十七",          // エラー: 数値に変換できない文字列
});

if (!result.success) {
  // result.error.format() を使うと、フィールドごとにエラーメッセージを整理して取得できます
